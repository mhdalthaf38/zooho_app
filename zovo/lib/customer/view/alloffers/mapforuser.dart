...
Map<String, String> extractAddressComponents(String address) {
  ...
  // Assigning values based on the number of parts available
  if (parts.isNotEmpty) result["District"] = parts[0]; // First part is usually District
  if (parts.length > 1) result["State"] = parts[1];    // Second part is State
  if (parts.length > 2) result["Pincode"] = parts[2];  // Third part is Pincode
  if (parts.length > 3) result["Country"] = parts[3];  // Fourth part is Country
  ...
  Future<void> _saveLocationToFirebase(LatLng location) async {
    try {
      ...
      BlocProvider.of<UserlocationBloc>(context).add(locationt(city: District!,state: state!,locationName: area!,longitude: location.longitude,latitude: location.latitude));
            ...
            options: MapOptions(
                onTap: (tapPosition, point) async {
                  ...
                  String address = await _getAddressFromLatLng(point);
                   final specifyAddress = await extractAddressComponents(address);
                  setState(() {
                      city= specifyAddress['City'];
        area =specifyAddress['Area'];
        District =specifyAddress['District'];
        state = specifyAddress['State'];
                    _selectedLocation = point;
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child:  ElevatedButton.icon(
                            ...
                            label:isloading?CircularProgressIndicator(color: AppColors.secondaryCream,): Text('Confirm Your Location'),
